...
describe("POST/DELETE /api/harnesses/opencode/providers (repo scope, fs)", () => {
  ...
  it("POST rejects an unknown provider id with 400", async () => {
    const { POST } = await loadRoute();
    const res = await POST(mockBody({ providerId: "not-a-real-provider", scope: "repo", repoRoot }));
  ...
  it("POST never writes a secret to the committed file (AC-7/AC-10)", async () => {
    const { POST } = await loadRoute();
    await POST(
      mockBody({ providerId: "openrouter", apiKey: "sk-leak", token: "ghp_leak", scope: "repo", repoRoot })
    );
    const raw = fs.readFileSync(repoSettingsPath(repoRoot), "utf-8");
  ...
  it("POST validates a gateway endpoint must be https/localhost", async () => {
    ...
    const bad = await POST(
      mockBody({ providerId: "openrouter", endpoint: "ftp://evil.example.com", scope: "repo", repoRoot })
    );
    expect(bad.status).toBe(400);
    const ok = await POST(
      mockBody({ providerId: "openrouter", endpoint: "https://gw.example.com/v1", scope: "repo", repoRoot })
    );
  ...
  it("GET ?configured=1 reports the scoped configured providers + counts", async () => {
    ...
    await POST(mockBody({ providerId: "anthropic", models: ["claude-opus-4-8"], scope: "repo", repoRoot }));
  ...
  it("DELETE removes a configured provider", async () => {
    ...
    const res = await DELETE(
      mockGet({ providerId: "anthropic", scope: "repo", repoRoot })
    );
    expect(res.status).toBe(200);
    const body = await res.json();
